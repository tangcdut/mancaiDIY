<template>
  <view class="page">
    <view v-if="!addresses || addresses.length === 0" class="empty">
      ...
      <view class="empty-text">No addresses yet / 暂无地址</view>
      <view class="empty-tips">Add an address for faster checkout / 添加地址方便下单</view>
    </view>
    ...
    <view v-else class="address-list">
      ...
      >
        <view class="addr-header">
          ...
          <view v-if="addr.isDefault" class="default-tag">Default / 默认</view>
        </view>
        <view class="addr-detail">
          {{ addr.address || addr.detail }}, {{ addr.city }}{{ addr.state ? ', ' + addr.state : '' }}, {{ addr.zip || '' }}, {{ addr.country || '' }}
        </view>
    ...
    <view class="bottom-bar">
      <button class="add-btn" @click="handleAdd">+ Add New Address / 添加新地址</button>
    </view>
  </view>
</template>
...
async function handleSetDefault(id) {
  try {
    uni.showLoading({ title: 'Setting... / 设置中...' })
    const res = await addressSetDefault(id)
    uni.hideLoading()
    if (res && res.ok) {
      uni.showToast({ title: 'Set successfully / 设置成功', icon: 'success' })
      loadAddresses()
    }
  } catch (e) {
    uni.hideLoading()
...
function handleDelete(id) {
  uni.showModal({
    title: 'Confirm / 确认删除',
    content: 'Delete this address? / 确定要删除这个地址吗？',
    confirmColor: '#e54d42',
    success: async (res) => {
      if (res.confirm) {
        try {
          uni.showLoading({ title: 'Deleting... / 删除中...' })
          const result = await addressDelete(id)
          uni.hideLoading()
          if (result && result.ok) {
            uni.showToast({ title: 'Deleted / 已删除', icon: 'success' })
            loadAddresses()
          }
        } catch (e) {
          uni.hideLoading()
...
.empty-tips { font-size: 26rpx; color: #999; }

.address-list { padding: 24rpx; }
...
.addr-name { font-size: 32rpx; font-weight: 700; color: #333; margin-right: 16rpx; }
.addr-phone { font-size: 28rpx; color: #666; margin-right: auto; }
...
.addr-actions { display: flex; gap: 16rpx; justify-content: flex-end; }
...
.action-btn.delete { background: #ffebee; color: #e54d42; }

...
export function getProductList(categoryId) {
  // 商品列表无需登录，海外版传递 region=overseas 获取USD价格
  return get(API_PATHS.PRODUCT_LIST, { categoryId, region: 'overseas' }, false)
...
export function getProductDetail(id) {
  // 商品详情无需登录，海外版传递 region=overseas 获取USD价格
  return get(API_PATHS.PRODUCT_DETAIL, { id, region: 'overseas' }, false)
...
export function aiRecommend(prompt, useTarot = false) {
  return post(API_PATHS.AI_RECOMMEND, { prompt, useTarot }, true, 60000)
}
...
export function updateAddress(data) {
  return put(API_PATHS.ADDRESS_UPDATE, data)
}
...
export function uploadFile(filePath, scene) {
  return new Promise((resolve, reject) => {
    // 海外版为游客下单，走免登录上传接口（/guest/**，无需登录 token，避免 401）
    let url = API_BASE_URL + '/guest/common/upload'
    if (scene) {
      url += `?scene=${scene}`
    }
...
// 登录 (海外版H5：不走微信登录，直接拒绝)
export const loginWithWeixinCode = (code, profile) => {
  console.warn('WeChat login is not available in overseas H5 version')
  return Promise.reject(new Error('WeChat login not available'))
}
...
/**
 * 免登录创建订单（海外版）
 * @param {Object} data - GuestOrderCreateDTO
 * @returns {Promise} { orderId, orderNo, amount }
 */
export function createGuestOrder(data) {
  return post('/guest/order/create', data, false)
}

/**
 * PayPal支付（免登录）
 * @param {String} orderNo - 订单号
 * @param {String} currencyCode - 货币代码 (默认 USD)
 * @returns {Promise} { paypalOrderId, approvalUrl }
 */
export function paypalPayment(orderNo, currencyCode = 'USD') {
  return post(`/guest/order/paypalPayment?orderNo=${orderNo}&currencyCode=${currencyCode}`, {}, false)
}
 ...
 * @returns {Promise} { isPaid, status, statusText }
 */
export function checkGuestPaymentStatus(orderNo) {
  return get('/guest/order/paymentStatus', { orderNo }, false)
}
 ...
 * @returns {Promise}
 */
export function guestOrderDetail(orderNo) {
  return get('/guest/order/detail', { orderNo }, false)
}

/**
 * 免登录查询订单列表
 * @param {Object} params - { email, orderNo }
 * @returns {Promise}
 */
export function guestOrderList(params) {
  return get('/guest/order/list', params, false)
}

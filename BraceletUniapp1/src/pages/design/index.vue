<template>
  <view class="page">
    ...
    <view class="main-area">
      ...
      <view class="canvas-container">
        ...
        >
          ...
          <view
            ...
            :class="{
              'is-new': b.isNew,
              'is-pending': b._pending,
              'is-mirrored': b.mirrored,
              'is-pendant': isPendant(b),
              'is-dragging': draggingBeadId === b._id,

              'is-delete-target': isDragging && draggingBeadId === b._id && isInDeleteZone,
      ...
      <!-- 拖动覆盖层：v-show 预创建 DOM + GPU 层，避免首帧初始化开销 -->
      <view
        v-show="dragOverlayVisible"
        class="drag-overlay-bead"
        :style="dragOverlayPos"
      >
      ...
      >
        ...
        />
        <view v-else class="flying-bead-dot" :style="{ background: fb.color || '#e8e8e8' }" />
      </view>
      ...
      <view class="section-body">
        ...
        <scroll-view class="product-area" scroll-y :show-scrollbar="false" :scroll-top="productScrollTop" @scrolltolower="onProductScrollToLower">
          ...
          <view v-else class="product-list">
            ...
            >
              <view class="p-visual">
                <view class="p-bead" :style="{background: g.color || 'transparent'}" @longpress.stop="previewProductImage(g, $event)">
                <image
                  v-if="g.imageUrl"
                  :src="g.imageUrl"
                  mode="aspectFit"
...
function addBead(g, event) {
  ...
  // 先添加珠子到数组，但标记为 _pending（不可见），用于布局计算和位置查询
  const newBeadId = `b_${++beadIdCounter}`
  beads.value.push({
    _id: newBeadId,
    productId: g.id,
    title: g.title,
  ...
  nextTick(() => {
    ...
    query.select('#product-' + g.id).boundingClientRect()
    query.select('#bead-' + newBeadId).boundingClientRect()
    query.exec(([cardRect, beadRect]) => {
      // 起飞点：产品卡片中心
      let startX = '50vw'
      let startY = '80vh'
      if (cardRect) {
        startX = (cardRect.left + cardRect.width / 2) + 'px'
        startY = (cardRect.top + cardRect.height / 2) + 'px'
      }
      ...
      let endY = '40vh'
      if (beadRect) {
        endX = (beadRect.left + beadRect.width / 2) + 'px'
        endY = (beadRect.top + beadRect.height / 2) + 'px'
      }
      ...
      flyingBeads.value.push(flyBead)

      // 阶段2：实物图飞向目标珠子位置并淡出（动画时长 800ms）
      setTimeout(() => {
        const idx = flyingBeads.value.findIndex(f => f.id === flyId)
        if (idx >= 0) {
          flyingBeads.value[idx].style = `
            left: ${endX};
            top: ${endY};
            opacity: 0.92;
      ...
      // 阶段3：飞行结束 → 显示真实珠子 + 清理飞行层
      setTimeout(() => {
        // 珠子可见（移除 _pending，触发 pop-in）
        const bead = beads.value.find(b => b._id === newBeadId)
        if (bead) {
          bead._pending = false
          bead.isNew = true
          // 延迟清除 isNew 标记
          setTimeout(() => { bead.isNew = false }, 500)
        }
        // 清理飞行层
        const idx = flyingBeads.value.findIndex(f => f.id === flyId)
        if (idx >= 0) {
          flyingBeads.value.splice(idx, 1)
        }
      }, 820)
    })
  })
...
// 通过 _id 查找珠子在 beads（不含gap）中的索引
function findBeadIndexById(id) {
...
function calcTargetSlot(touch) {
  ...
  beadLayouts.value.forEach((bl, idx) => {
    let ln = bl.angle + Math.PI / 2
    ln = ln % (2 * Math.PI)
    if (ln < 0) ln += 2 * Math.PI
    let diff = Math.abs(ln - na)
...
function onBeadTouchStart(beadId, e) {
  ...
  if (swapMode.value) {
    if (firstSwapIndex.value === idx) {
      swapMode.value = false; firstSwapIndex.value = -1
      vibrate(); uni.showToast({ title: '已取消', icon: 'none', duration: 800 })
    } else {
      swapBeads(firstSwapIndex.value, idx)
      swapMode.value = false; firstSwapIndex.value = -1
      vibrate(); uni.showToast({ title: '已交换位置', icon: 'success', duration: 800 })
    }
    return
  }

  beadTouchActive.value = true
  draggingBeadId.value = beadId
...
function onTouchMove(e) {
  ...
  if (isDragging.value) {
    ...
    const inZone = canvasRect.value ? checkInDeleteZone(touch) : false
    if (inZone !== isInDeleteZone.value) {
      isInDeleteZone.value = inZone
    }
  }
}
...
function onTouchEnd(e) {
  ...
  dragOverlayImageUrl.value = ''
  dragOverlayColor.value = ''
  dragOverlayPos.value = ''
  isDragging.value = false
...
async function initData() {
  try {
    ...
    uni.showToast({ title: '数据加载失败，请下拉刷新', icon: 'none', duration: 3000 })
  }
}

function ensureInit() {
  // 不再自动弹出操作指南，用户可通过 "!" 按钮手动查看
  // 保留代码以便后续恢复
  // try {
  //   const hide = uni.getStorageSync('dontShowHelp')
...
.drag-overlay-bead .bead-img {
  width: 100%;
  height: 100%;
}

.drag-overlay-bead .bead-placeholder {
  width: 100%;
  height: 100%;
  border-radius: 50%;
}
...
.flying-bead-img {
  ...
  box-shadow: 0 0 0 3rpx rgba(0, 0, 0, 0.12), 0 10rpx 36rpx rgba(0, 0, 0, 0.35);
  display: block;
